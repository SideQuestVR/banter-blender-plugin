...
class SqAppApi:
    ...
    def load_data(self):
        ...
        self.check_code_timer = RepeatedTimer(
            10 if self.login_code is None else self.login_code.interval,
            self.check_login_code_complete,
        )
        if os.path.exists(path):
            ...
            self.token.access_token_expires_at = datetime.fromisoformat(
                self.token.access_token_expires_at.replace("Z", "+00:00")
            ).replace(tzinfo=None)
    ...
    def set_my_avatar(self):
        ...
        return SqUser(
            self.json_get("/v2/users/me/avatar/files", True)
        )  # put - body {"high_avatar_files_id": files_id, "low_avatar_files_id": files_id, "is_public": true}
    ...
    def get_login_code(self, scopes=None):
        ...
        response = json.loads(
            self.json_post(
                ...
                False,
                False,
                json.dumps({"client_id": self.config.client_id, "scopes": scopes}),
            )
        )
        self.login_code = SqLoginCode()
        self.login_code.__dict__.update(response)
        self.login_code.expires_at = datetime.fromisoformat(
            self.login_code.expires_at.replace("Z", "+00:00")
        ).replace(tzinfo=None)
        print(
            "Got login code: "
            + self.login_code.code
            + " with device id: "
            + self.login_code.device_id
        )
    ...
    def check_login_code_complete(self):
        ...
        if (
            datetime.now() - self._last_login_poll
        ).total_seconds() < self.login_code.interval:
            return False, None
        ...
        print("checking code...")
        response_json = self.json_post(
            "/v2/oauth/checkshortcode",
            False,
            False,
            json.dumps(
                {"code": self.login_code.code, "device_id": self.login_code.device_id}
            ),
        )
    ...
    def get_auth_token(self):
        ...
        if (
            datetime.utcnow()
            < self.token.access_token_expires_at - timedelta(minutes=1)
            and self.token.access_token
        ):
        ...
        response = self.post_form_encoded_string_no_auth(
            "/v2/oauth/token",
            "grant_type=refresh_token&refresh_token="
            + quote_plus(self.token.refresh_token)
            + "&client_id="
        ...
        new_auth = {
            ...
            "access_token_expires_at": datetime.fromisoformat(
                response["access_token_expires_at"].replace("Z", "+00:00")
            ).replace(tzinfo=None),
